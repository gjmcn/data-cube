...
* These methods *do not* convert the calling array to a cube (except for [`toCube`](#method_to_cube){.internal}).

* These methods can be called as standard functions. The [`dc`](?exported){.internal} function exported by Data-Cube is equivalent to the [`toCube`](#method_to_cube){.internal} method; other methods can be called as properties of `dc`. For example:
  ``` {.no-exec}
